...
EDGE2_DDOS_TEST = {'attackers': ['192.168.0.170','192.168.0.128','192.168.0.152','216.58.198.74'],'victims': ['192.168.0.170','192.168.0.128','255.255.255.255','216.58.198.74','255.255.255.250']}   

DOS2019_FLOWS = {'attackers': ['172.16.0.5'], 'victims': ['192.168.50.1', '192.168.50.6', '192.168.50.7', '192.168.50.8']}                                      

CoAP_DDoS_TEST =  {'attackers': ['192.168.1.2', '192.168.1.9', '192.168.1.10', '192.168.1.12', '192.168.1.21', '192.168.1.5', '192.168.1.13', '0.0.0.0', '192.168.1.1'],'victims': ['192.168.1.1', '192.168.1.12', '192.168.1.2', '192.168.1.5', '192.168.1.255', '192.168.1.9', '224.0.0.251']}
...
DDOS_ATTACK_SPECS = {
    ...
    'EDGE2DDOS': EDGE2_DDOS_TEST,
    'CoAP_DDoS': CoAP_DDoS_TEST,
}
...
def get_ddos_flows(attackers, victims):
    DDOS_FLOWS = {'attackers': [], 'victims': []}

    for ip in attackers:
        if '/' in ip:
            DDOS_FLOWS['attackers'] += [str(x) for x in ipaddress.IPv4Network(ip).hosts()]
        else:
            DDOS_FLOWS['attackers'].append(str(ip))

    for ip in victims:
        if '/' in ip:
            DDOS_FLOWS['victims'] += [str(x) for x in ipaddress.IPv4Network(ip).hosts()]
        else:
            DDOS_FLOWS['victims'].append(str(ip))
...
def main(argv):
    ...
    parser.add_argument('-t', '--dataset_type', nargs='+', type=str,
                        help='Type of the dataset. Available options are: DOS2017, DOS2018, DOS2019, SYN2020,EDGE2DDOS,CoAP_DDoS')
    ...
    if args.preprocess_folder is not None or args.preprocess_file is not None:
        ...
        if args.no_split == True: # don't split the dataset
            ...
            print('output file generation----111------')
            ...
            print('output file generation------222----')
